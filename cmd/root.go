...
import (
	...
	"github.com/dockyard/dockyard/internal/actions"
	"github.com/dockyard/dockyard/internal/api"
	"github.com/dockyard/dockyard/internal/flags"
	"github.com/dockyard/dockyard/internal/logging"
	"github.com/dockyard/dockyard/internal/meta"
	"github.com/dockyard/dockyard/internal/scheduling"
	"github.com/dockyard/dockyard/internal/util"
	"github.com/dockyard/dockyard/internal/webui"
	"github.com/dockyard/dockyard/pkg/container"
	"github.com/dockyard/dockyard/pkg/filters"
	"github.com/dockyard/dockyard/pkg/metrics"
	"github.com/dockyard/dockyard/pkg/notifications"
...
func init() {
	...
	rootCmd.PersistentFlags().Bool("web-ui", false, "Enable the Dockyard web dashboard")
	rootCmd.PersistentFlags().String("web-ui-host", "", "Web UI bind host (default: all interfaces)")
	rootCmd.PersistentFlags().String("web-ui-port", "8080", "Web UI port")
	rootCmd.PersistentFlags().String("web-ui-data", "/app/data", "Data directory for state persistence")
}
...
func NewRootCommand() *cobra.Command {
	return &cobra.Command{
		Use:    "dockyard",
		Short:  "Docker container update manager with web UI",
		Long:   "\nDockyard automatically updates running Docker containers whenever a new image is released.\nFeatures a web dashboard for managing updates, scheduling, and per-container control.\nMore information available at https://github.com/dockyard/dockyard/.",
		Run:    run,
		PreRun: preRun,
...
func run(command *cobra.Command, args []string) {
	...
	webUI, _ = command.PersistentFlags().GetBool("web-ui")
	webUIHost, _ = command.PersistentFlags().GetString("web-ui-host")
	webUIPort, _ = command.PersistentFlags().GetString("web-ui-port")
	webUIData, _ = command.PersistentFlags().GetString("web-ui-data")
...
func runMain(cfg types.RunConfig) int {
	...
	// Start the Dockyard web UI if enabled
	if webUI {
		webUIAddr := ":" + webUIPort
		if webUIHost != "" {
		...
		go func() {
			if err := webSrv.Start(ctx); err != nil {
				logrus.WithError(err).Error("Dockyard web UI failed")
			}
		}()
